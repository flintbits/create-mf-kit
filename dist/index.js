...
// lib/util/cliPrompts.js
import readline from "node:readline/promises";
import { stdin as input, stdout as output } from "node:process";
async function ask({ type = "text", name, message, validate: validate2, choices }) {
...
var CONTAINER_WEBPACK_PATH = path.resolve(
  process.cwd(),
...
async function createNewRemoteApp(appName) {
  ...
  try {
    await import_fs_extra.default.copy(templateAppDir, targetDir);
    ...
    let webpackContent = await import_fs_extra.default.readFile(webpackPath, "utf-8");
    webpackContent = webpackContent.replace(/port:\s*\d+/, `port: ${appPort}`).replace(/name:\s*["']app1["']/, `name: "${appName}"`);
    await import_fs_extra.default.writeFile(webpackPath, webpackContent);
    await updateContainerRemotes(appName, appPort);
    spinner.succeed(
      `Created new remote: ${source_default.magenta(appName)} on port ${source_default.yellow(
        appPort
      )}`
    );
...
async function updateContainerRemotes(appName, appPort) {
  try {
    let content = await import_fs_extra.default.readFile(CONTAINER_WEBPACK_PATH, "utf-8");
    ...
    if (!remotesRegex.test(content)) {
      console.log(
        source_default.red("Could not find remotes block in container webpack config.")
      );
      return;
    }
    content = content.replace(remotesRegex, (match, inside) => {
      if (inside.includes(`${appName}:`)) {
        console.log(
          source_default.yellow(`Remote "${appName}" already exists in container.`)
        );
        return match;
      }
...
${updated}}`;
    ...
    await import_fs_extra.default.writeFile(CONTAINER_WEBPACK_PATH, content);
    console.log(
      source_default.green(`  Updated container to include remote "${appName}"`)
    );
  } catch (err) {
    console.log(
      source_default.red("Failed to update container webpack config:"),
      err.message
    );
  }
}
...
// lib/util/execCommand.js
var import_cross_spawn = __toESM(require_cross_spawn(), 1);
var exeCommand = (commands, args2, options = {}) => {
  return new Promise((resolve, reject) => {
    const child = (0, import_cross_spawn.default)(commands, args2, {
      stdio: "inherit",
      cwd: options.cwd || process.cwd()
      // Run command in given dir
    });
    child.on("error", (err) => {
...
var TEMPLATE_BASE2 = path2.resolve(__dirname2, "../templates");
async function mfeScaffold() {
  try {
    ...
    const containerDir = path2.resolve(process.cwd(), projectName, "container");
    try {
      switch (containerFramework) {
        //Copy the base template from ./templates
        case "base":
          const selectedTemplateDir = path2.resolve(
            TEMPLATE_BASE2,
            `../templates/${shared.stateLib}`
          );
          await import_fs_extra2.default.copy(selectedTemplateDir, projectDir);
          break;
        ...
        case "react-vite":
          await exeCommand(
            ...
            {
              cwd: projectDir
              // Important to run inside projectDir
            }
          );
          break;
      ...
      console.log(
        ...
        source_default.yellow(projectDir)
      );
    } catch (err) {
    ...
    console.log(source_default.white(`  cd container >> npm install >> npm start`));
    console.log(source_default.cyan("\n In a new terminal:"));
    console.log(source_default.white(`  cd app1 >> npm install >> npm start`));
  } catch (err) {
    console.error(source_default.red("Failed to scaffold project:"), err.message);
    process.exit(1);
  }
}
